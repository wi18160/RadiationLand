...
public class Enemy extends Mob {
	...
	public Enemy(Level level, String name, int x, int y, int speed, int shirtcolor) {
		super(level, name, x, y, speed, shirtcolor);
	}

	private int colour = Colours.get(-1, 111, 000, 040);
	private int scale = 1;
	protected boolean isSwimming = false;
	private int tickCount = 0;
	int richtung = 0;
	int abstand = 100; // attacknähe
	...
	public void tick() {// ------------------60 MAL PRO SEKUNDE-------------------------------
		...
		if (alive) {
			...
			if (attackPlayer) {
				...
				if (Game.playerX != this.x || Game.playerY != this.y) { // FOLGE SPIELeR
